...
["out/extension.js", "out/webview.js", "out/models.js", "out/skills.js", "out/home.js"].forEach((f) => {
...
const html = webview.getWebviewHtml("n0nce", "default-src 'none'", "vscode-resource://logo.svg");
...
assert.ok(!html.includes("${") || html.includes("nonce"), "template interpolations resolved except documented");
...
assert.strictEqual(pkg.contributes.viewsContainers.activitybar[0].icon, "media/activity.svg");
...
assert.ok(CATALOG.some((m) => m.id === "bailu-2.8"), "bailu-2.8 present");
assert.ok(CATALOG.some((m) => m.id === "bailu-apex-openclaw"), "bailu-apex-openclaw present");
assert.ok(CATALOG.some((m) => m.id === "bailu-2.8-nvfp8"), "bailu-2.8-nvfp8 present");
...
const thinkEnum = pkg.contributes.configuration.properties["bailu.thinking"].enum;
assert.ok(thinkEnum.includes("instant") && thinkEnum.includes("max") && thinkEnum.includes("off"), "thinking enum coverage");

const extSrc = fs.readFileSync(path.join(root, "out/extension.js"), "utf8");
assert.ok(!extSrc.includes('require("./webtools")'), "must not load broken webtools module");
