// Internal audit script for v1.1.9 - not part of final package
...
function extractFencePath(header) {
  ...
  if (
    ...
    if (
      knownLibs.has(candidate.toLowerCase()) &&
...
for (let i = 0; i < 24; i++)
  msgs1.push({ role: i % 2 === 0 ? "user" : "assistant", content: "msg" + i });
trimHistory(msgs1);
if (msgs1.length !== 24 || msgs1[0].role !== "system")
  console.error("FAIL trimHistory: sys+24 case");
...
trimHistory(msgs2);
if (msgs2.length !== 24)
  console.error("FAIL trimHistory: no-sys 30 -> 24, got:", msgs2.length);
...
trimHistory(msgs3);
if (msgs3.length !== 24)
  console.error("FAIL trimHistory: 24 exact, got:", msgs3.length);
...
if (overCap.length) {
  console.log(
    "INFO: Models with maxOut > MAX_OUT_CAP (will be capped to " +
      MAX_OUT_CAP +
      "):",
  );
  overCap.forEach((m) => {
    console.log("  ", m.id, "=>", m.maxOut, "-> capped to", MAX_OUT_CAP);
  });
  console.log(
    "  RECOMMENDATION: Either raise MAX_OUT_CAP or correct catalog values.",
  );
} else {
...
let m = yearRx.exec(html);
const yearMatches = [];
while (m !== null) {
  yearMatches.push(Number(m[1]));
  m = yearRx.exec(html);
}
const staleYears = yearMatches.filter((y) => y < CURRENT_YEAR);
if (staleYears.length) {
  console.log(
    "WARN: Footer has static year(s) that may lag behind:",
    staleYears,
    "(current:",
    CURRENT_YEAR + ")",
  );
