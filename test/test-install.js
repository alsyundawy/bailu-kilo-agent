...
if (!fs.existsSync(vsixPath)) {
  console.log(
    "[TEST-INSTALL] VSIX not found, running build & package first...",
  );
  execFileSync(NPM_BIN, ["run", "compile"], {
    cwd: root,
    stdio: "inherit",
    env: SECURE_ENV,
  });
  execFileSync(NPM_BIN, ["run", "package"], {
    cwd: root,
    stdio: "inherit",
    env: SECURE_ENV,
  });
...
assert.ok(
  vsixStats.size > 20000,
  `VSIX package size (${vsixStats.size} bytes) looks too small`,
);
console.log(
  `[TEST-INSTALL] VSIX exists, size: ${(vsixStats.size / 1024).toFixed(2)} KB`,
);

// 2. Test ZIP archive integrity and inspect file listing
let fileList = "";
try {
  fileList = execFileSync(UNZIP_BIN, ["-Z", "-1", vsixPath], {
    encoding: "utf8",
    env: SECURE_ENV,
  });
} catch {
  // Fallback to tar if unzip -Z not supported
  fileList = execFileSync(TAR_BIN, ["-tf", vsixPath], {
    encoding: "utf8",
    env: SECURE_ENV,
  });
}
...
for (const m of mandatory) {
  const found = files.some(
    (f) =>
      f.toLowerCase() === m.toLowerCase() ||
      (m.endsWith("LICENSE") && f.startsWith("extension/LICENSE")),
  );
  assert.ok(found, `[TEST-INSTALL] Mandatory file missing from VSIX: ${m}`);
}
...
console.log(
  "[TEST-INSTALL] Exclusion check PASSED (clean packaging, zero source leaks).",
);
...
console.log(
  `[TEST-INSTALL] Extracting to simulated extension host location: ${tmpDir}`,
);

try {
  execFileSync(UNZIP_BIN, ["-q", vsixPath, "-d", tmpDir], {
    stdio: "pipe",
    env: SECURE_ENV,
  });
} catch {
  execFileSync(TAR_BIN, ["-xf", vsixPath, "-C", tmpDir], {
    stdio: "pipe",
    env: SECURE_ENV,
  });
}
...
assert.ok(
  fs.existsSync(installedExtDir),
  "Extracted 'extension' folder must exist",
);
...
assert.ok(
  Array.isArray(installedPkg.activationEvents),
  "activationEvents must be an array",
);
assert.ok(
  installedPkg.contributes?.views?.bailuAgent?.some(
    (v) => v.id === "bailuAgent.sidebar",
  ),
  "sidebar view contributed",
);
assert.ok(
  installedPkg.contributes?.viewsContainers?.activitybar?.some(
    (v) => v.id === "bailuAgent",
  ),
  "activitybar container contributed",
);
...
const mockVscode = {
  window: {
    registerWebviewViewProvider: (viewId, provider, options) => {
      ...
      return {
        dispose() {
          webviewProviderRegistered = null;
        },
      };
    },
  ...
  Uri: {
    ...
    joinPath: (base, ...segs) => ({
      fsPath: path.join(base.fsPath || base, ...segs),
      path: path.join(base.fsPath || base, ...segs),
    }),
    parse: (u) => ({ toString: () => u }),
  },
  env: {
...
assert.ok(
  fs.existsSync(installedExtEntry),
  `Entry point ${installedExtEntry} must exist`,
);

const extModule = require(installedExtEntry);
assert.strictEqual(
  typeof extModule.activate,
  "function",
  "ext.activate must be a function",
);
assert.strictEqual(
  typeof extModule.deactivate,
  "function",
  "ext.deactivate must be a function",
);
...
console.log(
  `[TEST-INSTALL] Subscriptions registered: ${mockSubscriptions.length}`,
);
assert.ok(
  mockSubscriptions.length >= 6,
  "Expected at least 6 subscriptions (provider, view registration, 5 commands)",
);
...
assert.doesNotThrow(
  () => newTaskHandler(),
  "bailuAgent.newTask handler must not throw",
);

const stopHandler = commandsRegistered.get("bailuAgent.stop");
assert.doesNotThrow(
  () => stopHandler(),
  "bailuAgent.stop handler must not throw",
);
...
async function runSimulation() {
  ...
  assert.ok(
    mockWebviewView.webview.html.length > 500,
    "Webview HTML must be populated",
  );
  assert.ok(
    mockWebviewView.webview.html.includes("<!DOCTYPE html>"),
    "Webview HTML valid structure",
  );
  assert.ok(
    mockWebviewView.webview.html.includes("Bailu Agent"),
    "Webview contains Bailu Agent",
  );
  assert.ok(
    mockWebviewView.webview.html.includes("ALSYUNDAWY IT SOLUTION"),
    "Webview contains branding",
  );
  assert.ok(
    mockWebviewView.webview.html.includes('id="modelBadgeBar"'),
    "Webview contains model badge bar",
  );
  ...
  assert.ok(
    Array.isArray(initMsg.catalog) && initMsg.catalog.length >= 38,
    "Init message includes 38+ models catalog",
  );

  // Verify models.js in extracted directory
  const installedModels = require(
    path.join(installedExtDir, "out", "models.js"),
  );
  assert.ok(
    installedModels.CATALOG.length >= 38,
    "Catalog must contain 38 models",
  );
  assert.ok(
    installedModels.CATALOG.some((m) => m.id === "bailu-2.8"),
    "bailu-2.8 model verified",
  );
  assert.ok(
    installedModels.CATALOG.some((m) => m.id === "bailu-apex-openclaw"),
    "bailu-apex-openclaw model verified",
  );
  ...
  console.log(
    "[TEST-INSTALL] Simulated installation & extension lifecycle test PASSED successfully!",
  );
}
