...
function getWebviewScript(nonce: string): string {
  ...
  const scriptLines: string[] = [
    ...
    String.raw`  s = s.replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, function(_,txt,url){ return '<a href="' + escHtml(url) + '" target="_blank" rel="noopener">' + escHtml(txt) + '</a>'; });`,
    "  return s;",
    "}",
    ...
    "    var headers = tableRows[0].split('|').map(function(h){ return h.trim(); }).filter(Boolean);",
    "    headers.forEach(function(h){ th += '<th>' + inlineMd(escHtml(h)) + '</th>'; });",
    "    th += '</tr></thead><tbody>';",
    "    for (var i = 2; i < tableRows.length; i++) {",
    "      var cells = tableRows[i].split('|').map(function(c){ return c.trim(); }).filter(Boolean);",
    "      th += '<tr>';",
    "      cells.forEach(function(c){ th += '<td>' + inlineMd(escHtml(c)) + '</td>'; });",
    "      th += '</tr>';",
    "    }",
    "    th += '</tbody></table>';",
    "    html += th;",
    ...
    "    var msgs2 = $('msgs'); if (msgs2) msgs2.scrollTop = msgs2.scrollHeight;",
    "  }",
    "  if (m.type === 'done') {",
    "    activeStreamEl = null;",
...
export function getWebviewHtml(nonce: string, csp: string, logoUri: string): string {
